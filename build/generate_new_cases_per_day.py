...
from build.constants import NEW_CASES_PER_DAY_PATH
...
def main():
    ...
    # TODO: Based on cross-checking with the hospitalization data published by TEHIK, the data
    #       listed in the manual_data.json file with the field name "intensive" appears to show the
    #       number of patients on ventilation. We should fix the terminology and make sure that
    #       the intensive and on ventilation statistics are being calculated correctly.
    intensive = list(
    ...
    # Dump JSON output
    save_as_json(NEW_CASES_PER_DAY_PATH, final_json)
